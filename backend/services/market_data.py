...
from .ibkr_market_data import (
    get_last_price as get_last_price_from_ibkr,
    get_kline_data as get_kline_data_from_ibkr,
    search_stock as ibkr_search_stock,
)
...
def get_last_price(symbol: str, market: str = "CRYPTO") -> float:
    ...
    try:
        if market in ["US", "IBKR"]:
            price = get_last_price_from_ibkr(symbol)
            if price and price > 0:
                logger.info(f"Got real-time price for {key} from IBKR: {price}")
                cache_price(symbol, market, price)
                return price
            raise Exception(f"Failed to get IBKR price: {price}")
        else:
        ...
        logger.error(f"Failed to get price: {err}")
...
def get_kline_data(symbol: str, market: str = "CRYPTO", period: str = "1d", count: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        if market in ["US", "IBKR"]:
            data = get_kline_data_from_ibkr(symbol, period, count)
            if data:
                logger.info(f"Got K-line data for {key} from IBKR, total {len(data)} items")
                return data
            raise Exception("Failed to get IBKR K-line data")
        else:
        ...
        logger.error(f"Failed to get K-line data: {err}")
